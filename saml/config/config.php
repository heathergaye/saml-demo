...
// NOTE: this is the default configuration, copied (more or less) from
// config-templates/config.php. All SAML demo customisations are at the bottom.
$config = [
...
//Force the right port number - https://github.com/simplesamlphp/simplesamlphp/issues/450
$isSecure = false;
...
$_SERVER['SERVER_PORT'] = $isSecure ? '443' : '80';
$config['baseurlpath'] = "{$REQUEST_PROTOCOL}://{$_SERVER['HTTP_HOST']}/simplesaml/";

// CUSTOM SAML CONFIGURATION FOR IRONSTAR OR LANDO. Change this as required.
$config['technicalcontact_name'] = "SAML Demo";
$config['technicalcontact_email'] = "CHANGE-THIS@technocrat.com.au";
...
$config['logging.format'] = '%date{%Y-%m-%dT%H:%i:%s%o} mesg=simplesamlphp: %level %stat[%trackid] %srcip %msg';
...
// Get lando configuration.
if (file_exists('/app/web/sites/default/settings.lando.php')) {
  // Initialise some extra vars in settings file to prevent warnings.
  $app_root = $site_path = '';
  $databases = [];
  include '/app/web/sites/default/settings.lando.php';
...
// Get DB configuration from ironstar.
if (file_exists('/tokaido/config/drupal/sites/default/settings.ironstar.php')) {
  $databases = [];
  include '/tokaido/config/drupal/sites/default/settings.ironstar.php';
...
$config['store.sql.username'] = $dbuser;
$config['store.sql.password'] = $dbpass;

$config['enable.saml20-idp'] = TRUE;

$config['metadatadir'] = __DIR__ . '/../metadata-config';
